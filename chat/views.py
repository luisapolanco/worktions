...
class Inbox(View):
    def get(self, request):
        inbox = Channel.objects.filter(userchannel__user__in=[request.user.id])
...
class DetailMs(LoginRequiredMixin, ChannelFormMixin, DetailView):
    ...
    def get_object(self, *args, **kwargs):
        ...
        if username == my_username:
            my_channel, _ = Channel.objects.get_or_create_current_user_channel(self.request.user)
            return my_channel
        if channel == None:
            raise Http404
...
def private_messages(request, username, *args, **kwargs):
    ...
    channel_users = channel.userchannel_set.all().values("user__username")
    print(channel_users)
    channel_message = channel.channelmessage_set.all()
    print(channel_message.values("text"))
    return HttpResponse(f"Nuestro Canal - {channel.id}")